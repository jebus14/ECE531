...
//#define DEFAULT_URL              "<IP Adress>:8000"
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--url") == 0) {
            if (i + 1 < argc) {
                ...
                printf("Error: No URL specified after --url\n");
                return INIT_ERR;
            }
    ...
    if (curl) {
        ...
        if (request_type == 1) {  // POST
            if (!message) {
                printf("Error: No message provided for POST request.\n");
                return INIT_ERR;
            }
            ...
            if (!message) {
                printf("Error: No message provided for PUT request.\n");
                return INIT_ERR;
            }
